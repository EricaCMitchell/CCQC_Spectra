...
class Gaussian(Lineshape):
    ...
    :math:`G(\omega-\omega_{ab}) = \frac{1}{\gamma\sqrt{2\pi}}\exp{-\frac{(\omega-\omega_{ab})^2}{2\gamma^2}}`
    """
    ...
    def lineshape(self, x_0: float) -> np.ndarray:
        ...
        """
        prefactor = 1.0 / (self.gamma(x_0) * np.sqrt(2.0 * np.pi))
        exponent = -0.5 * ((self.domain - x_0) / self.gamma(x_0))**2

        return prefactor * np.exp(exponent)

    def maximum(self, x_0: float) -> float:
        return 1.0 / (self.gamma(x_0) * np.sqrt(2.0 * np.pi))
...
def prefactor_opa() -> float:
    ...
    e_0 = 8.854187817E-12 
    au_to_Coulomb_centimeter = 1.6021766208E-19 * 5.2917721067E-11 * 1.0E2 
...
def abs_cross_sec(dmec):
    r"""Converts the decadic molar extinction coefficient
    to the absorption cross section.
    Notes
    -------
    This function implements the calculation of the following:
    .. math::
        \sigma(\omega) = \frac{1000 \ln(10) \varepsilon(\omega)}{N_{\mathrm{A}}}
    in macroscopic units of :math: `\mathrm{L}\cdot\mathrm{cm}^{-1}`
    Parameters
    -------
    dmec 
    ...
    ------
    acs 
	Numpy array of the absorption cross section values
	:math: `\sigma(\omega)`
    """
    N_A = 6.022140857E+23

    acs = np.array([(1000 * np.log(10) * eps) / N_A for eps in dmec]) 
    return acs 
...
def prefactor_ecd() -> float:
    ...
    e_0 = 8.854187817E-12 
    au_to_Coulomb_centimeter = 1.6021766208E-19 * 5.2917721067E-11 * 1.0E2 
    au_to_Joule_inverse_Tesla = 2.0 * 9.274009994E-24 * 1.0E2
...
def spectrum(*,
    ...
    * Conversion of the broadening parameter :math:`\gamma`.
    ...
    # Converts :math: `\varepsilon(\omega)` to absorption cross-section in Mb
    acs = abs_cross_sec(y) * 1.0E18
    stk_Mb = abs_cross_sec(sticks) * 1.0E18

    return {"convolution": {"x": x, "y": acs}, "sticks": {"poles": poles, "residues": stk_Mb}}
